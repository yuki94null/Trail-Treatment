...
data modify storage yk.lib:inputs Item set from storage yk.lib:global CurrentItem
data modify storage yk.lib:global tmp.3 set from storage yk.lib:global CurrentItem

$data modify storage yk.lib:global tmp.3.Slot set value $(RecieveIndex)b
...
execute store result storage yk.lib:global tmp.3.count int 1.0 run scoreboard players get $Tmp yk.lib.global

tellraw @a [{nbt:"tmp.3",storage:"yk.lib:global"}]

$execute store success storage yk.lib:global OverRided byte 1.0 run data modify storage yk.lib:outputs DeltaRecieveItems[{Slot:$(RecieveIndex)b}] set from storage yk.lib:global tmp.3

execute if data storage yk.lib:global {OverRided:0b} run data modify storage yk.lib:outputs DeltaRecieveItems append from storage yk.lib:global tmp.3
